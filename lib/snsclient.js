...
function SNSClient (opts, verifySubscription, cb) {
  // opts and verifiySubscription are entirely optional, but cb is not
  ...
  if (typeof verifySubscription === 'function') {
    cb = verifySubscription;
    verifySubscription = true;
  }

  return function SNSClient (req, res) {
    var
      tmpmessage = req.payload,
      message = JSON.parse(tmpmessage);
    ...
    validateRequest(opts, message, function (err) {
      ...
      if (message.Type === 'SubscriptionConfirmation' && verifySubscription) {
        return https.get(url.parse(message.SubscribeURL), function (r) {
          cb(null, r);
        }).on('error', function (e) {
          cb(e);
        });
      ...
      if (message.Type === 'SubscriptionConfirmation') {
        return cb(null, message.SubscribeURL);
      }
